...
const handleAddProductToLocalCart = (state, action) => {
  ...
  let targetProduct = action.payload;
  // 查找商品 如果找到 返回商品 如果没有找到 返回undefined
  const product = newState.find(product => product.id === targetProduct.id);
  if (product) {
    // 商品已经存在于购物车中
...
export default createReducer({
  ...
  // 删除本地购物车中的商品
  [deleteProductFromCart]: handleDeleteProductFromLocalCart,
  // 更新购物车中商品的数量
