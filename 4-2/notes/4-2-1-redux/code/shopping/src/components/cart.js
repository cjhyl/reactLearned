...
class Cart extends Component {
  ...
  render () {
    const { carts, deleteProductFromCart, loadCarts } = this.props;
          ...
          carts.map((product) => (
            <div className="cart-row" key={product.id}>
              ...
              <div className="cart-quantity cart-column">
                <input value={product.count} onChange={e => this.changeProductNumber(product.id, e)} className="cart-quantity-input" type="number" />
                <button onClick={async () => {
                  await deleteProductFromCart(product.id);
                  loadCarts();}
                } className="btn btn-danger" type="button">删除</button>
              </div>
            </div>
          ))
        }
